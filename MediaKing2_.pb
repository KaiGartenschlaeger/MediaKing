...
IncludeFile "E:\Documents\PureBasic\Source\Include\fmodex.pbi"
...
Enumeration 
  #Plugin_Show = #WM_USER + 101
  #Plugin_Play = #WM_USER + 102
EndEnumeration
...
;Mutex
Global iMutex_PL.i       = CreateMutex()
Global iMutex_ML.i       = CreateMutex()
...
;FMOD
Global lFMOD_Version.i
Global lFMOD_Result.i
Global lFMOD_System.i
Global lFMOD_Channel.i
Global lFMOD_Sound.i
Global lFMOD_SubTracks.i
Global lFMOD_SoundTemp.i
Global lFMOD_SoundLen.i
Global lFMOD_SpectrumMode.i
Global lFMOD_InitEquilizer.i
Global iChannelCallback_NextTrack.i
Global lFMOD_EffHPass.i
Global lFMOD_EffLPass.i
Global lFMOD_EffEcho.i
Global lFMOD_EffFlange.i
Global lFMOD_EffDistortion.i
Global lFMOD_EffChorus.i
Global lFMOD_EffParamq.i
Global lFMOD_OutputMode.i
Global lFMOD_OldOutputMode.i
Global lFMOD_OutputDevice.i
Global iFMOD_SpeakerMode.i
...
Procedure.i IsPlaying()
 ...
 If lFMOD_Channel <> 0
  lFMOD_Result = FMOD_Channel_IsPlaying(lFMOD_Channel, @iResult)
 EndIf
...
Procedure.i IsPaused()
 ...
 If lFMOD_Channel <> 0
  lFMOD_Result = FMOD_Channel_GetPaused(lFMOD_Channel, @iResult)
 EndIf
...
Procedure.i GetPosition()
 ...
 If lFMOD_Channel <> 0
  lFMOD_Result = FMOD_Channel_GetPosition(lFMOD_Channel, @iResult, #FMOD_TIMEUNIT_MS)
...
Procedure.i GetMute()
 ...
 If lFMOD_Channel <> 0
  lFMOD_Result = FMOD_Channel_GetMute(lFMOD_Channel, @iResult)
  If lFMOD_Result <> #FMOD_OK
   iResult = -1
  EndIf
 EndIf
...
Procedure.i SetVolume()
 ...
 GadgetToolTip(#G_TB_Main_IA_Volume, "Volume: " + Str(GetGadgetState(#G_TB_Main_IA_Volume)) + "%")
 
 If lFMOD_System > 0
  
  If IsPlaying()
   lFMOD_Result = FMOD_Channel_SetVolume(lFMOD_Channel, fVolume)
   AddLogEntry("FMOD_Channel_SetVolume: " + FMOD_ErrorString(lFMOD_Result))
  EndIf
...
Procedure.i SetPanel()
 Protected fPanel.f, sToolTip.s
 
 fPanel = (Pref\panel - 100) / 100
 
 If fPanel < 0
  sToolTip = "100% / " + Str((1 + fPanel) * 100) + "%"
 ElseIf fPanel = 0
  sToolTip = "100% / 100%"
 Else
  sToolTip = Str((1 - fPanel) * 100) + "% / 100%"
 EndIf
 
 If lFMOD_System > 0
  
  If IsPlaying()
   lFMOD_Result = FMOD_Channel_SetPan(lFMOD_Channel, fPanel)
   AddLogEntry("FMOD_Channel_SetPan: " + FMOD_ErrorString(lFMOD_Result))
  EndIf
 
 EndIf
 
 ProcedureReturn lFMOD_Result
EndProcedure
 ...
 If lFMOD_System > 0
  ...
  If CurrPlay\length > 0
   ...
   If GetAsyncKeyState_(#VK_LBUTTON) = 0
    ...
    fPos = (GetGadgetState(#G_TB_Main_IA_Position) * CurrPlay\length) / #PosAccuracy
       
    If fPos >= CurrPlay\length
     fPos = CurrPlay\length - 1
    EndIf
    If fPos < 0
     fPos = 0
    EndIf
    
    lFMOD_Result = FMOD_Channel_SetPosition(lFMOD_Channel, fPos, #FMOD_TIMEUNIT_MS)
    AddLogEntry("FMOD_Channel_SetPosition: " + FMOD_ErrorString(lFMOD_Result))
...
Procedure SetSpeed()
 ...
 Protected fPanel.f, frequency.f
 
 fPanel = Pref\speed / 100
 
 If lFMOD_System > 0
  
  If IsPlaying()
   lFMOD_Result = FMOD_Sound_GetDefaults(lFMOD_Sound, @frequency, 0, 0, 0)
   AddLogEntry("FMOD_Sound_GetDefaults: " + FMOD_ErrorString(lFMOD_Result))
   lFMOD_Result = FMOD_Channel_SetFrequency(lFMOD_Channel, fPanel * frequency)
   AddLogEntry("FMOD_Channel_SetFrequency: " + FMOD_ErrorString(lFMOD_Result))
  EndIf
...
Procedure SetPause(Bool)
 ...
 If lFMOD_System > 0
  If IsPlaying()
   ...
   If Bool < 1 : Bool = 0 : Else : Bool = 1 : EndIf
   
   lFMOD_Result = FMOD_Channel_SetPaused(lFMOD_Channel, Bool)
   AddLogEntry("FMOD_Channel_SetPaused: " + FMOD_ErrorString(lFMOD_Result))
   SetToolBarButtonState(#Toolbar_Main1, #Mnu_Main_TB1_Pause, Bool)
    
   ;PluginSystem
   ForEach Plugin()
    If IsLibrary(Plugin()\handle) <> 0
     If GetFunction(Plugin()\handle, #DLL_FuncPause)
      CallFunction(Plugin()\handle, #DLL_FuncPause, @Bool)
     EndIf
    EndIf
   Next
...
Procedure SetPlay()
 ...
 If lFMOD_System > 0
  ...
  If lFMOD_Sound > 0 And GetPaused()
   SetPause(0)
   ProcedureReturn -1
  EndIf
  ...
  If ListSize(PlayList()) > 0
   If GetGadgetState(#G_LI_Main_PL_PlayList) = -1
    PlayList_ChangeSelect(0, 1)
   Else
    If ListSize(PlayList()) >= GetGadgetState(#G_LI_Main_PL_PlayList) + 1
     PlayList_ChangeSelect(GetGadgetState(#G_LI_Main_PL_PlayList), 1)
    EndIf
   EndIf
  EndIf
...
Procedure.i SetStop()
 ...
 Protected iCurrSound.i
 
 If lFMOD_System > 0
  
  iChannelCallback_NextTrack = 1
 
  lFMOD_Result = FMOD_Channel_GetCurrentSound(lFMOD_Channel, @iCurrSound)
  
  If lFMOD_Channel > 0 And iCurrSound > 0
   lFMOD_Result = FMOD_Channel_Stop(lFMOD_Channel)
   If lFMOD_Result = #FMOD_OK
    lFMOD_Channel = 0
   EndIf
  EndIf
  
  iChannelCallback_NextTrack = 0
...
Procedure.s ReadTagData(*TagData, TagLen, TagType)
 ...
 Select TagType
  Case #FMOD_TAGDATATYPE_BINARY
  Case #FMOD_TAGDATATYPE_INT
   sResult = Str(PeekL(*TagData))
  Case #FMOD_TAGDATATYPE_FLOAT
   sResult = StrF(PeekF(*TagData), 2)
  Case #FMOD_TAGDATATYPE_STRING
   sResult = PeekS(*TagData, TagLen, #PB_Ascii)
  Case #FMOD_TAGDATATYPE_STRING_UTF16
   If TagLen > 2
    If PeekW(*TagData) = -257
     sResult = PeekS(*TagData + 2, -1, #PB_UTF16) ;Ignore BOM
     If Len(sResult) > TagLen: sResult = Left(sResult, TagLen): EndIf
    EndIf
   EndIf
  Case #FMOD_TAGDATATYPE_STRING_UTF16BE
   If TagLen > 2
    If PeekW(*TagData) = -2
     sResult = PeekS(*TagData + 2, -1, #PB_UTF16BE) ;Ignore BOM
     If Len(sResult) > TagLen: sResult = Left(sResult, TagLen): EndIf
    EndIf
   EndIf
  Case #FMOD_TAGDATATYPE_STRING_UTF8
   sResult = PeekS(*TagData, -1, #PB_UTF8)
   If Len(sResult) > TagLen: sResult = Left(sResult, TagLen): EndIf
  Case #FMOD_TAGDATATYPE_CDTOC
 EndSelect
...
Procedure.i ReadTagEx(Sound)
 Protected lNext.i, lNumTags.i, sResult.s, lResult.i
 ...
 ;/////////////////
 ;ReadTag
 lFmod_Result = FMOD_Sound_GetNumTags(Sound, @lNumTags, 0)
 AddLogEntry("FMOD_Sound_GetNumTags: " + FMOD_ErrorString(lFMOD_Result))
 If lFmod_Result = #FMOD_OK
  
  For lNext = 0 To lNumTags - 1
   
   lFmod_Result = FMOD_Sound_GetTag(Sound, 0, lNext, @Fmod_Tag)
   AddLogEntry("FMOD_Sound_GetTag: " + FMOD_ErrorString(lFMOD_Result))
   If lFmod_Result = #FMOD_OK
    
    AddElement(Tag())
    Tag()\type   = Fmod_Tag\datatype
    Tag()\name   = PeekS(Fmod_Tag\name)
    Tag()\string = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
    
    Select FMOD_TAG\type
     
     ;MP3 V1
     Case #FMOD_TAGTYPE_ID3V1
      ;Debug PeekS(Fmod_Tag\name) + ": " + Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
      Select PeekS(Fmod_Tag\name)
       Case "TITLE"   : ReadTag\title   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "ARTIST"  : ReadTag\artist  = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "ALBUM"   : ReadTag\album   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "YEAR"    : ReadTag\year    = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "COMMENT" : ReadTag\comment = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "TRACK"   : ReadTag\track   = GetFirstNum(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "GENRE"   : ReadTag\genre   = GetMP3Genre(Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype)))
      EndSelect
     ...
     Case #FMOD_TAGTYPE_ID3V2
      ...
      Select PeekS(Fmod_Tag\name)
       Case "TIT2"
        sResult = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
        If sResult <> "": ReadTag\title = sResult: EndIf
       Case "TPE1"
        sResult = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
        If sResult <> "": ReadTag\artist = sResult: EndIf
       Case "TALB"
        sResult = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
        If sResult <> "": ReadTag\album = sResult: EndIf
       Case "TYER"
        sResult = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
        If sResult <> "": ReadTag\year = sResult: EndIf
       Case "COMM"
        sResult = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
        If sResult <> "": ReadTag\comment = sResult: EndIf
       Case "TRCK"
        lResult = GetFirstNum(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
        If lResult <> -1: ReadTag\track = lResult: EndIf
       Case "TCON"
        sResult = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
        If sResult <> "": ReadTag\genre = GetMP3Genre(sResult): EndIf
      EndSelect
     ...
     Case #FMOD_TAGTYPE_ASF
      ...
      Select PeekS(Fmod_Tag\name)
       Case "TITLE"          : ReadTag\title   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "AUTHOR"         : ReadTag\artist  = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "WM/AlbumTitle"  : ReadTag\album   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "WM/Year"        : ReadTag\year    = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "DESCRIPTION"    : ReadTag\comment = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "WM/TrackNumber" : ReadTag\track   = GetFirstNum(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "WM/Genre"       : ReadTag\genre   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
      EndSelect
     
     ;Shoutcast
     Case #FMOD_TAGTYPE_SHOUTCAST
      ;Debug "#FMOD_TAGTYPE_SHOUTCAST - " + PeekS(Fmod_Tag\name) + ": " + Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
      Select PeekS(Fmod_Tag\name)
       Case "icy-name"    : ReadTag\title   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "TITLE"       : ReadTag\title   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "ARTIST"      : ReadTag\artist  = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "icy-url"     : ReadTag\album   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "icy-genre"   : ReadTag\genre   = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
       Case "icy-notice1" : ReadTag\comment + Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype)) + #CRLF$
       Case "icy-notice2" : ReadTag\comment + Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype)) + #CRLF$
      EndSelect
     
     Case #FMOD_TAGTYPE_ICECAST
      ;Debug "#FMOD_TAGTYPE_ICECAST - " + PeekS(Fmod_Tag\name) + ": " + Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
     
     ;Playlist
     Case #FMOD_TAGTYPE_PLAYLIST
      Select PeekS(Fmod_Tag\name)
       Case "FILE" : AddElement(PlayListQuene()) : PlayListQuene() = Trim(ReadTagData(Fmod_Tag\_data, Fmod_Tag\datalen, Fmod_Tag\datatype))
      EndSelect
 ...
 ;Check
 ReadTag\title   = CheckSaveString(ReadTag\title)
 ReadTag\artist  = CheckSaveString(ReadTag\artist)
 ReadTag\album   = CheckSaveString(ReadTag\album)
 ReadTag\year    = CheckSaveString(ReadTag\year)
 ReadTag\comment = CheckSaveString(ReadTag\comment)
 ReadTag\genre   = CheckSaveString(ReadTag\genre)
 
 ;Length
 lFmod_Result = FMOD_Sound_GetLength(Sound, @ReadTag\length, #FMOD_TIMEUNIT_MS)
 AddLogEntry("FMOD_Sound_GetLength: " + FMOD_ErrorString(lFMOD_Result))
 If lFMOD_Result <> #FMOD_OK
  ReadTag\length = -1
 EndIf
 
 ;Format
 lFMOD_Result = FMOD_Sound_GetFormat(Sound, @ReadTag\type, @ReadTag\format, @ReadTag\channels, @ReadTag\bits)
 AddLogEntry("FMOD_Sound_GetFormat: " + FMOD_ErrorString(lFMOD_Result))
 If lFMOD_Result <> #FMOD_OK
  ReadTag\type     = -1
  ReadTag\format   = -1
  ReadTag\channels = -1
 ...
 ;Bitrate
 lFMOD_Result = FMOD_Sound_GetLength(Sound, @lResult, #FMOD_TIMEUNIT_RAWBYTES)
 AddLogEntry("FMOD_Sound_GetLength: " + FMOD_ErrorString(lFMOD_Result))
 If lFMOD_Result = #FMOD_OK And ReadTag\length > 0 And lResult > 0
  iTemp = ReadTag\length / 1000
  If iTemp > 0
   ReadTag\bitrate = lResult / (iTemp) / 1000 * 8
  EndIf
 Else
  ReadTag\bitrate = -1
 EndIf
 ...
 ;Defaults
 lFMOD_Result = FMOD_Sound_GetDefaults(Sound, @ReadTag\frequency, @ReadTag\volume, @ReadTag\pan, @ReadTag\priority)
 AddLogEntry("FMOD_Sound_GetDefaults: " + FMOD_ErrorString(lFMOD_Result))
 If lFMOD_Result <> #FMOD_OK
  ReadTag\frequency = -1
  ReadTag\volume    = -1
...
Procedure RefreshTags()
 ...
 If lFMOD_System > 0 And lFMOD_Sound > 0
  
  Protected numtagsupdated.i, numtags.i
  
  lFmod_Result = FMOD_Sound_GetNumTags(lFMOD_Sound, @numtags, @numtagsupdated)
  If lFMOD_Result = #FMOD_OK And numtagsupdated > 0
   
   AddLogEntry("Refresh Tags")
   
   ReadTagEx(lFMOD_Sound)
   
   If ReadTag\title <> CurrPlay\title
    CurrPlay\title = ReadTag\title
    SetGadgetText(#G_TX_Main_IA_TitleC, CurrPlay\title)
   EndIf
    
   If ReadTag\artist <> CurrPlay\artist
    CurrPlay\artist = ReadTag\artist
    SetGadgetText(#G_TX_Main_IA_ArtistC, CurrPlay\artist)
   EndIf
...
Procedure CloseMedia()
 ...
 If lFMOD_Sound > 0
  lFMOD_Result = FMOD_Sound_Release(lFMOD_Sound)
  AddLogEntry("FMOD_Sound_Release: " + FMOD_ErrorString(lFMOD_Result))
  If lFMOD_Result = #FMOD_OK
   lFMOD_Sound = 0
  EndIf
 EndIf
...
Procedure.i PlayMedia_StartTimer()
 Protected iOpenState.i
 Protected iPercentBuffered.i
 Protected iStarving.i
  
 lFMOD_Result = FMOD_Sound_GetOpenState(lFMOD_Sound, @iOpenState, @iPercentBuffered, @iStarving)
 If lFMOD_Result = #FMOD_OK
  ...
  Case #FMOD_OPENSTATE_CONNECTING
   If GetGadgetText(#G_TX_Main_IA_TitleC) <> "Verbindung.."
    SetGadgetText(#G_TX_Main_IA_TitleC, "Verbindung..")
   EndIf
   
  ;Buffering
  Case #FMOD_OPENSTATE_BUFFERING
   If GetGadgetText(#G_TX_Main_IA_TitleC) <> "Pufferung.."
    SetGadgetText(#G_TX_Main_IA_TitleC, "Pufferung..")
   EndIf
   
  ;Loading
  Case #FMOD_OPENSTATE_LOADING
   If GetGadgetText(#G_TX_Main_IA_TitleC) <> "Öffnen.."
    SetGadgetText(#G_TX_Main_IA_TitleC, "Öffnen..")
   EndIf
   
  ;Seeking
  Case #FMOD_OPENSTATE_SEEKING
   If GetGadgetText(#G_TX_Main_IA_TitleC) <> "Seeking.."
    SetGadgetText(#G_TX_Main_IA_TitleC, "Seeking..")
   EndIf
    
  ;Streaming
  Case #FMOD_OPENSTATE_STREAMING
   If GetGadgetText(#G_TX_Main_IA_TitleC) <> CurrPlay\title And iStarving = 0
    SetGadgetText(#G_TX_Main_IA_TitleC, CurrPlay\title)
   EndIf
       
  ;Error
  Case #FMOD_OPENSTATE_ERROR
   If GetGadgetText(#G_TX_Main_IA_TitleC) <> "Fehler"
    SetGadgetText(#G_TX_Main_IA_TitleC, "Fehler")
   EndIf
   
  ;Ready
  Case #FMOD_OPENSTATE_READY
   CompilerIf #PB_Compiler_Debugger
    If IsPlaying() : MessageRequester("", "Playing allrady? Q.q") : EndIf
   CompilerEndIf
    
   lFMOD_Result = FMOD_System_PlaySound(lFMOD_System, #FMOD_CHANNEL_FREE, lFMOD_Sound, 1, @lFMOD_Channel)
   AddLogEntry("FMOD_System_PlaySound: " + FMOD_ErrorString(lFMOD_Result))
   If lFMOD_Result = #FMOD_OK
    ;Callback
    lFMOD_Result = FMOD_Channel_SetCallback(lFMOD_Channel, @ChannelCallback())
    AddLogEntry("FMOD_Channel_SetCallback: " + FMOD_ErrorString(lFMOD_Result))
    
    AddLogEntry("Play File: " + CurrPlay\path)
    
    ;/////////////////////////////////
    ;Sound and Channel Attribute
    SetVolume()
    SetPanel()
    SetSpeed()
    SetEquilizer(-1)
    ...
    ;Title
    CurrPlay\title      = ReadTag\title
    If Trim(CurrPlay\title) = ""
     CurrPlay\title = GetFileNamePart(CurrPlay\path)
    EndIf
    ;Artist
    CurrPlay\artist     = ReadTag\artist
    ;Album
    CurrPlay\album      = ReadTag\album
    ;Year
    CurrPlay\year       = ReadTag\year
    ;Track
    CurrPlay\track      = ReadTag\track
    ;Comment
    CurrPlay\comment    = ReadTag\comment
    ;Genre
    CurrPlay\genre      = ReadTag\genre
    ;Length
    lFmod_Result = FMOD_Sound_GetLength(lFMOD_Sound, @CurrPlay\length, #FMOD_TIMEUNIT_MS)
    If lFMOD_Result <> #FMOD_OK
     CurrPlay\length = -1
    EndIf
    ;Format
    lFMOD_Result = FMOD_Sound_GetFormat(lFMOD_Sound, @CurrPlay\type, @CurrPlay\format, @CurrPlay\channels, @CurrPlay\bits)
    If lFMOD_Result <> #FMOD_OK
     CurrPlay\type     = -1
     CurrPlay\format   = -1
     CurrPlay\channels = -1
    ...
    EndIf
    ;Defaults
    lFMOD_Result = FMOD_Sound_GetDefaults(lFMOD_Sound, @CurrPlay\frequency, @CurrPlay\volume, @CurrPlay\pan, @CurrPlay\priority)
    If lFMOD_Result <> #FMOD_OK
     CurrPlay\frequency = -1
     CurrPlay\volume    = -1
     CurrPlay\pan       = -1
    ...
    EndIf
    ;StartTime
    CurrPlay\starttime  = timeGetTime_()
    
    ;/////////////////////////////////
    ;Refresh Info
    SetGadgetText(#G_TX_Main_IA_TitleC, CurrPlay\title)
    SetGadgetText(#G_TX_Main_IA_ArtistC, CurrPlay\artist)
    SetGadgetText(#G_TX_Main_IA_AlbumC, CurrPlay\album)
    SetGadgetText(#G_TX_Main_IA_GenreC, CurrPlay\genre)
    If CurrPlay\length <> -1
    ...
    ForEach Plugin()
     If IsLibrary(Plugin()\handle) <> 0
      If GetFunction(Plugin()\handle, #DLL_FuncPlay)
       CallFunction(Plugin()\handle, #DLL_FuncPlay, @CurrPlay)
      EndIf
     EndIf
    Next
    ...
    ;ShoutcastName
    ForEach Shoutcast()
     If LCase(Shoutcast()\url) = LCase(CurrPlay\path)
    ...
    ;Copy ClipboardInfo
    If Pref\clipboard > 0
     SetClipboardText("MK_TI:" + CurrPlay\path + "|" + CurrPlay\title + "|" + CurrPlay\artist + "|" + CurrPlay\album + "|" + Str(CurrPlay\track) + "|" + Str(CurrPlay\length) + "|" + CurrPlay\year + "|" + CurrPlay\comment + "|" + CurrPlay\genre + "|" + Str(CurrPlay\frequency))
    EndIf
   EndIf
   
   KillTimer_(0, Timer_StartStream)
   Timer_StartStream = 0
...
Procedure.i PlayMedia(File$, ErrorMessage = 0)
 If (FileSize(File$) > 0 Or LCase(Left(File$, 4)) = "http") And Timer_StartStream = 0
  
  ;Add to MediaLibary
  If Pref\addplayfiles > 0 And Pref\medialibpath <> "" And FileSize(Pref\medialibpath) = -2
   If LCase(Pref\medialibpath) = LCase(Left(File$, Len(Pref\medialibpath)))
    MediaLib_AddFile(File$)
   EndIf
  EndIf
  ...
  ;CreateSound
  lFMOD_Result = FMOD_System_CreateStream(lFMOD_System, @File$, #FMOD_UNIQUE|#FMOD_SOFTWARE|#FMOD_ACCURATETIME|#FMOD_LOOP_OFF|#FMOD_2D|#FMOD_NONBLOCKING, 0, @lFMOD_Sound)
  AddLogEntry("FMOD_System_CreateStream: " + FMOD_ErrorString(lFMOD_Result))
  If lFMOD_Result = #FMOD_OK
   CurrPlay\path = File$
   Timer_StartStream = SetTimer_(0, 0, 10, @PlayMedia_StartTimer())
   ProcedureReturn #FMOD_OK
  Else
   ;/////////////////////////////////
   ;Failed
   If ErrorMessage > 0
    MsgBox_Exclamation("Datei '" + File$ + "' kann nicht wiedergegeben werden" + #CR$ + "'" + FMOD_ErrorString(lFMOD_Result) + "'", "Ungültige Datei")
    ProcedureReturn -1
   EndIf
  EndIf
...
Procedure CurrPlay_Information()
 If lFMOD_System > 0 And (FileSize(CurrPlay\path) > 0 Or Left(LCase(CurrPlay\path), 4) = "http")
  lFmod_Result = FMOD_System_CreateSound(lFMOD_System, @CurrPlay\path, #FMOD_SOFTWARE|#FMOD_CREATESTREAM|#FMOD_2D|#FMOD_OPENONLY, 0, @lFMOD_SoundTemp)
  If lFmod_Result = #FMOD_OK
   ShowWindow_TitleInfo()
    
   ReadTagEx(lFMOD_SoundTemp)
    
   SetGadgetText(#G_SR_TitleInfo_Path,    CurrPlay\path)
   GadgetToolTip(#G_SR_TitleInfo_Path,    CurrPlay\path)
   
   If Trim(ReadTag\title) = ""
    ReadTag\title = GetFileNamePart(CurrPlay\path)
   EndIf
...
Procedure.i PlayList_ChangeSelect(Sel, ErrorMessage = 0)
 If lFMOD_System > 0 And Sel > -1 And ListSize(PlayList()) > Sel
  SelectElement(PlayList(), Sel)
  
  iChannelCallback_NextTrack = 1
  
  If PlayMedia(PlayList()\path, ErrorMessage) = #FMOD_OK
   PlayList_ResetRandoms()
   PlayList()\random = 1
   
   SetGadgetItemColor(#G_LI_Main_PL_PlayList, Sel, #PB_Gadget_BackColor, Pref\color_sel_bg)
   SetGadgetItemColor(#G_LI_Main_PL_PlayList, Sel, #PB_Gadget_FrontColor, Pref\color_sel_tx)
   lSelect = Sel + 1
   SetGadgetState(#G_LI_Main_PL_PlayList, -1)
...
Procedure.i PlayList_BackTrack()
 If lFMOD_System > 0 And ListSize(PlayList()) > 1
  Protected lNSel.i
  
  PlayList_ResetRandoms()
  
  If ListSize(PlayList()) > 1
   If GetToolBarButtonState(#Toolbar_Main2, #Mnu_Main_TB2_Random) <> 0
    ;Random
    Repeat
     lNSel = Random(ListSize(PlayList()) - 1)
     SelectElement(PlayList(), lNSel)
    Until lNSel <> lSelect - 1 And PlayList()\random = 0
   Else
    ;Normal
    If lSelect > 1
     lNSel = lSelect - 2
    Else
     lNSel = ListSize(PlayList()) - 1
    EndIf
   EndIf
  Else
   lNSel = 0
  ...
  ;Change
  If ListSize(PlayList()) >= lNSel
   ProcedureReturn PlayList_ChangeSelect(lNSel)
  EndIf
 ...
 If lFMOD_System > 0 And ListSize(PlayList()) > 1
  ...
  If ListSize(PlayList()) > 1
   If GetToolBarButtonState(#Toolbar_Main2, #Mnu_Main_TB2_Random) <> 0
    ;Random
    Repeat
     iNewSelect = Random(ListSize(PlayList()) - 1)
     SelectElement(PlayList(), iNewSelect)
    Until iNewSelect <> lSelect - 1 And PlayList()\random = 0
   Else
    ;Normal
    If lSelect >= ListSize(PlayList())
     iNewSelect = 0
    Else
     iNewSelect = lSelect
    EndIf
  ...
  ;Change
  If ListSize(PlayList()) >= iNewSelect
   ProcedureReturn PlayList_ChangeSelect(iNewSelect)
  EndIf
...
Procedure PlayList_AddFile(Path$)
 ...
 If Path$ <> ""
  ...
  lFmod_Result = FMOD_System_CreateStream(lFMOD_System, @Path$, #FMOD_SOFTWARE|#FMOD_2D|#FMOD_OPENONLY, 0, @lFMOD_SoundTemp)
  AddLogEntry("FMOD_System_CreateSound: " + FMOD_ErrorString(lFMOD_Result))
  If lFmod_Result = #FMOD_OK
   ...
   If lFMOD_Result = #FMOD_OK
    ...
    If lType <> #FMOD_SOUND_TYPE_PLAYLIST And lType <> #FMOD_SOUND_TYPE_UNKNOWN
     ...
     ;Title
     PlayList()\title    = ReadTag\title
     If Trim(ReadTag\title) = ""
      PlayList()\title = GetFileNamePart(Path$)
     EndIf
     ;Artist
     PlayList()\artist   = ReadTag\artist
     ;Album
     PlayList()\album    = ReadTag\album
     ;Year
     PlayList()\year     = ReadTag\year
     ;Track
     PlayList()\track    = ReadTag\track
     ;Comment
     PlayList()\comment  = ReadTag\comment
     ;Genre
     PlayList()\genre    = ReadTag\genre
     ;Path
     PlayList()\path     = Path$
     ;Length
     lFmod_Result = FMOD_Sound_GetLength(lFMOD_SoundTemp, @PlayList()\length, #FMOD_TIMEUNIT_MS)
     If lFMOD_Result <> #FMOD_OK
      PlayList()\length = -1
     EndIf
               
     PlayList_AddItem(PlayList()\title, PlayList()\artist, PlayList()\album, PlayList()\track, PlayList()\length, PlayList()\Year)
     Playlist_RefreshAllTimes()
     PlayList_RefreshHeader()
     
    ElseIf lType = #FMOD_SOUND_TYPE_PLAYLIST
     
     ;Playlist
     ClearList(PlayListQuene())
     
     ReadTagEx(lFMOD_SoundTemp)
     
     ForEach PlayListQuene()
      Playlist_AddFile(PlayListQuene())
     Next
...
Procedure PlayList_Informationen()
 If lFMOD_System > 0 And ListSize(PlayList()) >= GetGadgetState(#G_LI_Main_PL_PlayList)
  ...
  If FileSize(PlayList()\path) > 0 Or Left(LCase(PlayList()\path), 4) = "http"
   lFmod_Result = FMOD_System_CreateSound(lFMOD_System, @PlayList()\path, #FMOD_SOFTWARE|#FMOD_CREATESTREAM|#FMOD_2D|#FMOD_OPENONLY, 0, @lFMOD_SoundTemp)
   If lFmod_Result = #FMOD_OK
    ShowWindow_TitleInfo()
    
    ReadTagEx(lFMOD_SoundTemp)
    
    SetGadgetText(#G_SR_TitleInfo_Path,    PlayList()\path)
    GadgetToolTip(#G_SR_TitleInfo_Path,    PlayList()\path)
    If Trim(ReadTag\title) = ""
     ReadTag\title = GetFileNamePart(PlayList()\path)
    EndIf
    SetGadgetText(#G_SR_TitleInfo_Title,   ReadTag\title)
    GadgetToolTip(#G_SR_TitleInfo_Title,   ReadTag\title)
    SetGadgetText(#G_SR_TitleInfo_Artist,  ReadTag\artist)
...
Procedure Open_NetStream(Flags)
 Protected sShoutCastName.s, sShoutCastURL.s, lResult.i, lAddServer.i
 
 sShoutCastURL = Trim(GetGadgetText(#G_SR_InternetStream_URL))
 If sShoutCastURL <> "" And Left(LCase(sShoutCastURL), 7) = "http://" And Len(sShoutCastURL) > 7
  
  ;ClearPlayList
  If Flags & #NetStream_Clear And ListSize(PlayList()) > 0
   Playlist_Remove(-1)
  EndIf
  
  lResult = Playlist_AddFile(sShoutCastURL)
  If lResult = #FMOD_OK
   ...
   If ListSize(PlayList()) > 0 And Flags & #NetStream_Play
    PlayList_ChangeSelect(0)
   EndIf
   
  Else
   MsgBox_Exclamation(FMOD_ErrorString(lResult), "Fehlgeschlagen")
  EndIf  
...
Procedure FMOD_Timer()
 
 If lFMOD_System <> 0
  
  ;SystemUpdate
  FMOD_System_Update(lFMOD_System)
  ...
  ;OpenState
  Protected iStarving.i, iPercentBuffered.i
  
  If lFMOD_Sound > 0
   lFMOD_Result = FMOD_Sound_GetOpenState(lFMOD_Sound, 0, @iPercentBuffered, @iStarving)
   If lFMOD_Result = #FMOD_OK
    ...
    If iStarving = 1
     If GetMute() = 0
      If GetGadgetText(#G_TX_Main_IA_TitleC) <> "Pufferung.."
       SetGadgetText(#G_TX_Main_IA_TitleC, "Pufferung..")
      EndIf
      FMOD_Channel_SetMute(lFMOD_Channel, 1)
     EndIf
    Else
     If GetMute() = 1
      If GetGadgetText(#G_TX_Main_IA_TitleC) <> CurrPlay\title
       SetGadgetText(#G_TX_Main_IA_TitleC, CurrPlay\title)
      EndIf
      FMOD_Channel_SetMute(lFMOD_Channel, 0)
     EndIf
    EndIf
...
Procedure.i FM_ChangeOutputType()
 
 If lFMOD_System > 0
  
   Protected iCurrMode.i
   
   If IsGadget(#G_CB_Pref_OutputMode)
    iCurrMode = GetGadgetState(#G_CB_Pref_OutputMode)
   Else
    iCurrMode = Pref\outputmode
   EndIf
  
   Select iCurrMode
    Case 0 , -1 : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_AUTODETECT
    Case 1      : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_NOSOUND
    Case 2      : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_WAVWRITER
    Case 3      : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_DSOUND
    Case 4      : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_WINMM
    Case 5      : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_OPENAL
    Case 6      : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_WASAPI
    Case 7      : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_ASIO
    Default     : lFMOD_OutputMode = #FMOD_OUTPUTTYPE_AUTODETECT
   EndSelect
  
   lFMOD_Result = FMOD_System_SetOutput(lFMOD_System, lFMOD_OutputMode)
   AddLogEntry("FMOD_System_SetOutput: " + FMOD_ErrorString(lFMOD_Result))
...
Procedure.i FM_ChangeOutputDriver()

 If lFMOD_System > 0
 
  lFMOD_Result = FMOD_System_SetDriver(lFMOD_System, Pref\outputdriver)
  AddLogEntry("FMOD_System_SetDriver: " + FMOD_ErrorString(lFMOD_Result))
...
Procedure.i FM_ChangeSoftwareFormat()
 
 If lFMOD_System > 0
 
  Protected iSamplerate.i, iResampleMethod
  
  If Pref\outputrate = -1 : Pref\outputrate = 0 : EndIf
  Select Pref\outputrate
   Case 0  : iSamplerate = 48000
   Case 1  : iSamplerate = 41100
   Case 2  : iSamplerate = 22050
   Case 3  : iSamplerate = 11025
   Case 4  : iSamplerate =  8000
   Default : iSamplerate = 48000
  EndSelect
  
  If Pref\interpolation = -1 : Pref\interpolation = 1 : EndIf
  Select Pref\interpolation
   Case 0  : #FMOD_DSP_RESAMPLER_NOINTERP
   Case 1  : #FMOD_DSP_RESAMPLER_LINEAR
   Case 2  : #FMOD_DSP_RESAMPLER_CUBIC
   Case 3  : #FMOD_DSP_RESAMPLER_SPLINE
   Default : #FMOD_DSP_RESAMPLER_LINEAR
  EndSelect
  
  FMOD_System_SetSoftwareFormat(lFMOD_System, iSamplerate, #FMOD_SOUND_FORMAT_PCM16, 0, 0, iResampleMethod)
  AddLogEntry("FMOD_System_SetSoftwareFormat: " + FMOD_ErrorString(lFMOD_Result))
...
Procedure.i FM_ChangeSpeakerMode()
 
 If lFMOD_System > 0
  
  Protected iSpeakerMode.i
 
  If Pref\speakermode = -1 : Pref\speakermode = 1 : EndIf
  Select Pref\speakermode
   Case 0  : iSpeakerMode = #FMOD_SPEAKERMODE_MONO
   Case 1  : iSpeakerMode = #FMOD_SPEAKERMODE_STEREO
   Case 2  : iSpeakerMode = #FMOD_SPEAKERMODE_QUAD
   Case 3  : iSpeakerMode = #FMOD_SPEAKERMODE_SURROUND
   Case 4  : iSpeakerMode = #FMOD_SPEAKERMODE_5POINT1
   Case 5  : iSpeakerMode = #FMOD_SPEAKERMODE_7POINT1
   Case 6  : iSpeakerMode = #FMOD_SPEAKERMODE_PROLOGIC
   Default : iSpeakerMode = #FMOD_SPEAKERMODE_STEREO
  EndSelect
  
  lFMOD_Result = FMOD_System_SetSpeakerMode(lFMOD_System, iSpeakerMode)
 
  If lFMOD_Result = #FMOD_ERR_OUTPUT_CREATEBUFFER
   Debug "STEREO"
   lFMOD_Result = FMOD_System_SetSpeakerMode(lFMOD_System, #FMOD_SPEAKERMODE_STEREO)
   If lFMOD_Result = #FMOD_OK
    SetGadgetState(#G_CB_Pref_SpeakerMode, 1)
   EndIf
  EndIf
...
Procedure.i FM_ChangeStreamBufferSize()
 
 If lFMOD_System > 0
 
  If Pref\streambuffer <= 0
   Pref\streambuffer = 16384
  EndIf
  
  lFMOD_Result = FMOD_System_SetStreamBufferSize(lFMOD_System, Pref\streambuffer, #FMOD_TIMEUNIT_RAWBYTES)
  AddLogEntry("FMOD_System_SetStreamBufferSize: " + FMOD_ErrorString(lFMOD_Result))
...
Procedure.i FM_RefreshDriverList()
 Protected iNext.i
 
 If FM_CloseSystem() = #FMOD_OK
  
  lFMOD_Result = FMOD_System_Create(@lFMOD_System)
  AddLogEntry("FMOD_System_Create: " + FMOD_ErrorString(lFMOD_Result))
  
  ;OutputMode
  If lFMOD_Result = #FMOD_OK
   
   lFMOD_Result = FM_ChangeOutputType()
  ...
  If lFMOD_Result = #FMOD_OK
   ...
   Protected lFmod_DriversNum.i, lFMOD_DName.s = Space(500), lFMOD_DNameLen.i = 500
 
   ClearList(Driver())
   If IsGadget(#G_CB_Pref_OutputDriver)
    ClearGadgetItems(#G_CB_Pref_OutputDriver)
   EndIf
 
   lFMOD_Result = FMOD_System_GetNumDrivers(lFMOD_System, @lFmod_DriversNum)
   If lFMOD_Result = #FMOD_OK
    For iNext = 0 To lFmod_DriversNum - 1
     
     lFMOD_Result = FMOD_System_GetDriverInfo(lFMOD_System, iNext, @lFMOD_DName, @lFMOD_DNameLen, 0)
     If lFMOD_Result = #FMOD_OK
      AddElement(Driver())
      Driver() = lFMOD_DName
    ...
    Next
    If IsGadget(#G_CB_Pref_OutputDriver) And ListSize(Driver()) > 0
...
Procedure.i FM_InitEffects()
;  
;  If lFMOD_System > 0
;   
;   ;HighPass
;   lFMOD_Result = FMOD_System_CreateDSPByType(lFMOD_System, #FMOD_DSP_TYPE_HIGHPASS, @lFMOD_EffHPass)
;   AddLogEntry("FMOD_System_CreateDSPByType(lFMOD_EffHPass): " + FMOD_ErrorString(lFMOD_Result))
;   If lFMOD_Result = #FMOD_OK
;    If GetGadgetState(#G_CH_Main_SO_HPass) = 1 : ChangeEffects(#G_CH_Main_SO_HPass, 1) : EndIf
;   Else
;    SetGadgetState(#G_CH_Main_SO_HPass, 0) : DisableGadget(#G_CH_Main_SO_HPass, 1)
;   EndIf
;   ;LowPass
;   lFMOD_Result = FMOD_System_CreateDSPByType(lFMOD_System, #FMOD_DSP_TYPE_LOWPASS, @lFMOD_EffLPass)
;   AddLogEntry("FMOD_System_CreateDSPByType(lFMOD_EffLPass): " + FMOD_ErrorString(lFMOD_Result))
;   If lFMOD_Result = #FMOD_OK
;    If GetGadgetState(#G_CH_Main_SO_LPass) = 1 : ChangeEffects(#G_CH_Main_SO_LPass, 1) : EndIf
;   Else
;    SetGadgetState(#G_CH_Main_SO_LPass, 0) : DisableGadget(#G_CH_Main_SO_LPass, 1)
;   EndIf
;   ;Echo
;   lFMOD_Result = FMOD_System_CreateDSPByType(lFMOD_System, #FMOD_DSP_TYPE_ECHO, @lFMOD_EffEcho)
;   AddLogEntry("FMOD_System_CreateDSPByType(lFMOD_EffEcho): " + FMOD_ErrorString(lFMOD_Result))
;   If lFMOD_Result = #FMOD_OK
;    If GetGadgetState(#G_CH_Main_SO_Echo) = 1 : ChangeEffects(#G_CH_Main_SO_Echo, 1) : EndIf
;   Else
;    SetGadgetState(#G_CH_Main_SO_Echo, 0) : DisableGadget(#G_CH_Main_SO_Echo, 1)
;   EndIf
;   ;Flange
;   lFMOD_Result = FMOD_System_CreateDSPByType(lFMOD_System, #FMOD_DSP_TYPE_FLANGE, @lFMOD_EffFlange)
;   AddLogEntry("FMOD_System_CreateDSPByType(lFMOD_EffFlange): " + FMOD_ErrorString(lFMOD_Result))
;   If lFMOD_Result = #FMOD_OK
;    If GetGadgetState(#G_CH_Main_SO_Flange) = 1 : ChangeEffects(#G_CH_Main_SO_Flange, 1) : EndIf
;   Else
;    SetGadgetState(#G_CH_Main_SO_Flange, 0) : DisableGadget(#G_CH_Main_SO_Flange, 1)
;   EndIf
;   ;Disortion
;   lFMOD_Result = FMOD_System_CreateDSPByType(lFMOD_System, #FMOD_DSP_TYPE_DISTORTION, @lFMOD_EffDistortion)
;   AddLogEntry("FMOD_System_CreateDSPByType(lFMOD_EffDistortion): " + FMOD_ErrorString(lFMOD_Result))
;   If lFMOD_Result = #FMOD_OK
;    If GetGadgetState(#G_CH_Main_SO_Distortion) = 1 : ChangeEffects(#G_CH_Main_SO_Distortion, 1) : EndIf
;   Else
;    SetGadgetState(#G_CH_Main_SO_Distortion, 0): DisableGadget(#G_CH_Main_SO_Distortion, 1)
;   EndIf
;   ;Chorus
;   lFMOD_Result = FMOD_System_CreateDSPByType(lFMOD_System, #FMOD_DSP_TYPE_CHORUS, @lFMOD_EffChorus)
;   AddLogEntry("FMOD_System_CreateDSPByType(lFMOD_EffChorus): " + FMOD_ErrorString(lFMOD_Result))
;   If lFMOD_Result = #FMOD_OK
;    If GetGadgetState(#G_CH_Main_SO_Chorus) = 1 : ChangeEffects(#G_CH_Main_SO_Chorus, 1) : EndIf
;   Else
;    SetGadgetState(#G_CH_Main_SO_Chorus, 0): DisableGadget(#G_CH_Main_SO_Chorus, 1)
;   EndIf
;  
;  Else
;   lFMOD_Result = -1
...
Procedure.i FM_InitEquilizer()
 ...
 If lFMOD_System > 0
  ...
  For iNext = 0 To 9
   lFMOD_Result = FMOD_System_CreateDSPByType(lFMOD_System, #FMOD_DSP_TYPE_PARAMEQ, @Equilizer(iNext))
   AddLogEntry("FMOD_System_CreateDSPByType(EQ" + Str(iNext) + "): " + FMOD_ErrorString(lFMOD_Result))
   If lFMOD_Result <> #FMOD_OK
    MsgBox_Error("Fehler beim initialisieren des Equilizers")
    lFMOD_InitEquilizer = 0
    Break
   Else
    lFMOD_InitEquilizer = 1
   EndIf
  Next
...
Procedure.i FM_InitSystem()
 ...
 If FM_CloseSystem() = #FMOD_OK
  ...
  ;CheckVersion
  lFmod_Result = FMOD_System_GetVersion(lFMOD_System, @lFMOD_Version)
  AddLogEntry("FMOD_System_GetVersion: " + FMOD_ErrorString(lFMOD_Result))
  If lFMOD_Result = #FMOD_OK
   If lFMOD_Version < #FMOD_VERSION
    MsgBox_Error("Sie benutzen eine zu alte Version der 'fmodex.dll'" + #CRLF$ + "Sie benötigen die Version " + Hex(#FMOD_VERSION), "FMOD Warnung")
    lFMOD_Result = FM_CloseSystem()
   EndIf
  EndIf
  ...
  If lFMOD_Result = #FMOD_OK
   ...
   lFMOD_Result = FM_ChangeOutputDriver()
  ...
  If lFMOD_Result = #FMOD_OK
   ...
   lFMOD_Result = FM_ChangeSoftwareFormat()
  ...
  If lFMOD_Result = #FMOD_OK
   ...
   lFMOD_Result = FM_ChangeSpeakerMode()
  ...
  If lFMOD_Result = #FMOD_OK
   ...
   lFMOD_Result = FM_ChangeStreamBufferSize()
  ...
  If lFMOD_Result = #FMOD_OK
   ...
   lFmod_Result = FMOD_System_Init(lFMOD_System, #MaxChannels, #FMOD_INIT_NORMAL, 0)
   AddLogEntry("FMOD_System_Init: " + FMOD_ErrorString(lFMOD_Result))
 ...
 If lFMOD_Result <> #FMOD_OK
  ...
  MsgBox_Error("Fehler beim initialisieren des Systems sind aufgetreten")
  FM_CloseSystem()
...
; EnableXP
; EnableOnError
; UseIcon = Ressource\icon.ico
; Executable = MediaKing.exe
; CPU = 1
; CompileSourceDirectory
; EnableCompileCount = 5694
; EnableBuildCount = 797
; EnableExeConstant
; Watchlist = Save\Preferences_Time;Save\Medialibary;Save\Medialibary_Time;Save\Preferences
; Watchlist = WinSize(1)\posx;WinSize(1)\posy;iWinW_Main_Second;lSelect;iWinH_Main_Second
; Watchlist = Save\Playlist;iWinH_Main_Second;iWinW_Main_Second;Save\Shoutcast;Save\Shoutcast_Time
; Watchlist = iWinW_Main_Normal;lFMOD_Channel;lFMOD_Sound;Order\order;Order\event;Save\PlayList_Time
; IncludeVersionInfo
; VersionField0 = 0,0,3,9
; VersionField1 = 2,0,0,0
; VersionField2 = PureSoft
; VersionField3 = MediaKing
; VersionField4 = 2.00
; VersionField5 = 0.39
; VersionField6 = Mediaplayer mit FMOD System
; VersionField7 = mediaking.exe
; VersionField8 = mediaking.exe
; VersionField9 = Kai Gartenschläger, 2008
; VersionField11 = Kai Gartenschläger, 2008
